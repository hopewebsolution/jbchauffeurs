      ...
      <div class="x_panel">
        <div class="x_title">
          ...
          <div class="navbar-right">
              <a href="{{ route('admin.airports.add') }}" class="btn btn-primary"><i class="fa fa-plus"></i>&nbsp;Add New</a>
          </div>
...
<script>
  $(document).ready(function(){
    $('.searchInput').on('keyup', function(){
      ...
      $.ajax({
        url : "{{ route('admin.airports.search') }}"+search_key,
        method: 'GET',
      }).done(function (data){
        out_put.html(data);
      }).fail(function () {
    ...
    $(document).on("click", '.delete', function(event){ 
      ...
      var click_obj=$(this);
      var url="{{ route('admin.airports.delete') }}";
      var data={'id':id};
      if(confirm("Are You sure want to delete this?")){
          deleteAirport(url,data,click_obj);
      }
    });
  });
  function deleteAirport(url,data,click_obj){
    $(".loader_html").show();
    $.ajaxSetup({
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

...
<head>
    ...
    <link href="{{ asset('public/assets/front_assets/vendors/bootstrap-select/dist/css/bootstrap-select.css') }}"
    ...
    <link href="{{ asset('public/assets/front_assets/css/jcarousel.responsive.css') }}" rel="stylesheet"
        type="text/css" />
    ...
    <link href="{{ asset('public/assets/front_assets/vendors/font-awesome/css/font-awesome.css') }}" rel="stylesheet"
        type="text/css" />
...
<body class="">
                    ...
                    <div class="col-md-5 col-sm-4">
                        <div class="slct">
                            ...
                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                                <img src="{{ asset('public/assets/front_assets/images') }}/{{ $common->currCountry['flag'] ?? '' }}"
                                    alt="">
                                    ...
                                    @foreach ($common->countries as $country)
                                        <li class="{{ $country['short'] == Request::segment(1) ? 'active' : '' }}">
                                            <a href="{{ url('/') }}/{{ $country['short'] }}">
                                                <img src="{{ asset('public/assets/front_assets/images/') }}/{{ $country['flag'] }}"
                                                    alt="" /> {{ $country['name'] }}
                                            </a>
                                        </li>
                                    @endforeach
                ...
                <div class="row">
                        ...
                        <div class="topleft">
                            <a href="{{ route('user.home') }}">
                                @if ($site_settings->logo)
                                    <img src="{{ asset('public/assets/images') }}/{{ $site_settings->logo }}"
                                        alt="jbchauffeurs">
                                @else
                                    <img src="{{ asset('public/assets/front_assets/images/logo.png') }}"
                                        alt="jbchauffeurs">
                                @endif
                            </a>
                        </div>
                    ...
                    <div class="col-md-2">
                        <ul class="topbar-image hidden-sm hidden-xs">
                            <li>
                                <div class="top-lft">
                                    <img src="{{ asset('public/assets/front_assets/images/img-lft.png') }}"
                                        alt="left" width="100%" />
                                </div>
                            </li>
                        </ul>
                    ...
                    <div class="col-md-4 col-sm-6 text-right hidden-sm hidden-xs">
                        @if ($site_settings->header_img)
                            <img src="{{ asset('public/assets/images') }}/{{ $site_settings->header_img }}"
                                alt="jbchauffeurs" width="auto" class="mid-img">
                        @else
                            <img src="{{ asset('public/assets/front_assets/uploads/banner/-78.jpg') }}"
                                alt="" width="auto" class="mid-img" />
                        @endif
                    </div>
                ...
                <ul id="menu">
                    @if ($common->header_menus)
                        @foreach ($common->header_menus as $header_menu)
                            @php
                                $page_slug =
                                    $header_menu->page_type == 'custom_page'
                                        ? $header_menu->slug
                                        : $header_menu->page_type;
                            @endphp
                            <li><a href="{{ route('user.cmsPage', ['page_slug' => $page_slug]) }}"
                                    class="menulink">{{ $header_menu->name }}</a></li>
                        @endforeach
                    @endif
                ...
                <div class="row">
                        ...
                        <div class="footer-left">
                            @if ($common->footer_menus)
                                @foreach ($common->footer_menus as $footer_menu)
                                    @php
                                        $page_slug =
                                            $footer_menu->page_type == 'custom_page'
                                                ? $footer_menu->slug
                                                : $footer_menu->page_type;
                                    @endphp
                                    <a
                                        href="{{ route('user.cmsPage', ['page_slug' => $page_slug]) }}">{{ $footer_menu->name }}</a>
                                    |
                                @endforeach
                            @endif
                    ...
                    <div class="col-md-4 col-sm-6">
                        <div class="footer-right">
                            <img src="{{ asset('public/assets/front_assets/images/cards.png') }}"
                                alt="Payment Accepted" width="50%" />
                        </div>
                        <div class="clear"></div>
                    </div>
                </div>
    ...
    <script type="text/javascript"
        src="{{ asset('public/assets/front_assets/vendors/bootstrap-select/dist/js/bootstrap-select.js') }}"></script>
